...
class NewscapperAi():
    ...
    @agent
    def scrape_website(self) -> Agent:
        return Agent(
            config=self.agents_config['scrape_website'], # type: ignore[index]
    ...
    def retrive_news_task(self) -> Task:
        return Task(
            config=self.tasks_config['retrive_news_task'], # type: ignore[index]
        )

    @task
    def scrape_website_task(self) -> Task:
        return Task(
            config=self.tasks_config['scrape_website_task'], # type: ignore[index]
        )
    @task
